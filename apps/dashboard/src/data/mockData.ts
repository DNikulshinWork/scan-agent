...
export const initialVacancies: Vacancy[] = [
  {
    ...
    description:
      'Ищем опытного Fullstack-разработчика в продуктовую финтех-команду. Мы разрабатываем масштабируемую SaaS-платформу для автоматизации финансовой аналитики и отчетности. Основной фокус — развитие ядра платформы, оптимизация SQL-запросов и построение отказоустойчивых WebSocket-соединений.',
...
Готов оперативно обсудить проектные задачи. Резюме и открытый код доступны на https://github.com/DNikulshin.`,
    ...
    keySkills: ['TypeScript', 'Next.js', 'React', 'Node.js', 'NestJS', 'PostgreSQL', 'Prisma', 'Redis', 'Docker', 'WebSocket', 'REST API', 'Git'],
    responsibilities: [
      'Проектирование и разработка клиентской (Next.js 15 App Router) и серверной части (NestJS / Node.js) веб-платформы',
      'Построение высоконагруженного REST и WebSocket API для realtime-обновления графиков и котировок',
      'Оптимизация реляционной схемы базы данных PostgreSQL, профилирование медленных запросов и транзакций через Prisma',
      'Интеграция очередей задач (Redis / BullMQ) для фоновой обработки отчетов и парсинга',
      'Покрытие ключевой бизнес-логики unit- и e2e-тестами, участие в code review и декомпозиции задач',
    ],
    requirements: [
      'Коммерческий опыт разработки на TypeScript / JavaScript от 3 лет',
      'Глубокое понимание React 18/19, современного Next.js (App Router, Server Actions, SSR/SSG)',
      'Уверенное владение Node.js и фреймворком NestJS или Fastify',
      'Практический опыт работы с PostgreSQL (проектирование схем, индексы, миграции Prisma/TypeORM)',
      'Знание Redis, Docker и понимание принципов построения микросервисной архитектуры',
    ],
    conditions: [
      'Полностью удаленный формат работы (Remote first), свободный график без трекеров времени',
      'Официальное оформление по ТК РФ или через ИП / Самозанятость на выбор',
      'Белая заработная плата с фиксацией в валюте или рублях, пересмотр грейда каждые 6 месяцев',
      'ДМС со стоматологией с первого месяца, компенсация профильного обучения и коворкинга',
      'Предоставление мощного рабочего ноутбука (MacBook Pro M-серии) или компенсация амортизации',
    ],
  ...
  },
  {
    ...
    description:
      'В продуктовое подразделение Core Infrastructure ищем Backend-разработчика. Наш сервис обеспечивает быстрый сбор, валидацию и нормализацию сотен тысяч событий в реальном времени. Стек: Node.js (Fastify) и Python (FastAPI), PostgreSQL, Prisma, Redis, Docker, RabbitMQ.',
...
С удовольствием подключусь к решению задач по микросервисам и базам данных. Мой GitHub: https://github.com/DNikulshin`,
    ...
    keySkills: ['Node.js', 'Fastify', 'Python', 'FastAPI', 'PostgreSQL', 'Prisma', 'Redis', 'Docker', 'RabbitMQ', 'Playwright', 'Linux'],
    responsibilities: [
      'Разработка отказоустойчивых микросервисов на Fastify (Node.js) и FastAPI (Python)',
      'Проектирование и оптимизация структуры таблиц и индексов в PostgreSQL под высокие нагрузки',
      'Настройка очередей сообщений и асинхронных воркеров через Redis / RabbitMQ',
      'Интеграция внешних API, автоматизация сбора данных с защищенных источников (Playwright / Puppeteer)',
      'Настройка Docker-контейнеров, Docker Compose и пайплайнов деплоя CI/CD',
    ],
    requirements: [
      'Опыт разработки серверных приложений на Node.js или Python от 2-3 лет',
      'Уверенные навыки проектирования RESTful API и работы с PostgreSQL',
      'Понимание многопоточности, асинхронности (Event Loop / async-await) и работы с брокерами сообщений',
      'Опыт работы с Docker, умение конфигурировать окружение и писать понятные Dockerfile',
      'Английский на уровне чтения технической документации',
    ],
    conditions: [
      '100% удаленная занятость из любой локации, комфортный тайм-трекинг по спринтам',
      'Гибкое начало дня (с 9 до 12 по МСК)',
      'Оформление по ТК РФ / ИП с выплатами 2 раза в месяц без задержек',
      'Оплата профильных курсов, литературы и конференций',
    ],
    employment: 'Полная занятость',
    status: 'new',
    outcome: 'pending',
  ...
  },
  {
    ...
    description:
      'Ищем разработчика для развития крупной PWA-системы аналитики и оперативного управления. Мы строим интерфейсы, работающие в реальном времени с большими массивами данных, офлайн-режимом и мгновенным откликом.',
...
Буду рад пообщаться и выполнить тестовое задание. Портфолио: https://dnikulshin.github.io`,
    ...
    keySkills: ['React', 'Next.js', 'TypeScript', 'TanStack Query', 'Tailwind CSS', 'IndexedDB', 'WebSocket', 'PWA', 'Redux Toolkit'],
    responsibilities: [
      'Разработка модульных клиентских интерфейсов на React 19 и Next.js',
      'Реализация локального офлайн-кэширования на IndexedDB и оптимистичных мутаций через TanStack Query',
      'Подключение realtime-стриминга данных по WebSocket с автоматическим reconnect',
      'Адаптивная верстка с Tailwind CSS под мобильные экраны и десктопы',
      'Взаимодействие с бэкенд-командой для согласования OpenAPI / Swagger контрактов',
    ],
    requirements: [
      'Уверенное знание TypeScript и React (Custom Hooks, оптимизация рендеров React.memo / useMemo)',
      'Опыт работы с TanStack Query (React Query) и управлением серверным стейтом',
      'Понимание принципов Progressive Web Apps (Service Workers, Manifest, IndexedDB)',
      'Навыки аккуратной верстки на Tailwind CSS без лишних сторонних UI-библиотек',
      'Умение писать чистый, тестируемый и поддерживаемый код',
    ],
    conditions: [
      'Удаленная работа с возможностью посещения офиса в СПб по желанию',
      'Оформление по ТК РФ, аккредитованная IT-компания (льготная ипотека, отсрочка)',
      'Компенсация фитнеса и уроков английского языка',
      'Премирование по итогам успешных запусков квартала',
    ],
    employment: 'Полная занятость',
    status: 'applied',
    outcome: 'pending',
  ...
  },
  {
    ...
    description:
      'Ищем инженера в проект автоматизации логистики. Мы разрабатываем систему для водителей и диспетчеров: кроссплатформенное мобильное приложение на React Native (Expo) и бэкенд на Node.js (TypeScript, PostgreSQL).',
...
Готов поделиться деталями архитектуры и подключиться к развитию вашего продукта.`,
    ...
    keySkills: ['React Native', 'Expo', 'Node.js', 'TypeScript', 'PostgreSQL', 'WebSocket', 'Docker', 'Geolocation API'],
    responsibilities: [
      'Разработка мобильного приложения на React Native (Expo SDK) для iOS и Android',
      'Настройка фонового отслеживания геолокации и отправки телеметрии на сервер',
      'Разработка REST и WebSocket API на Node.js (TypeScript)',
      'Сборка и публикация билдов через Expo EAS Build и GitHub Actions',
      'Оптимизация энергопотребления и трафика мобильного клиента',
    ],
    requirements: [
      'Коммерческий опыт с React Native и платформой Expo от 1.5 лет',
      'Уверенная разработка бэкенда на Node.js / TypeScript',
      'Знание PostgreSQL и опыт проектирования таблиц для хранения временных рядов (геотреков)',
      'Опыт работы с WebSocket и обработкой сетевых сбоев/офлайн-режима',
    ],
    conditions: [
      'Полная удаленка, гибкий график',
      'Официальное трудоустройство или контракт',
      'Стабильная индексация заработной платы',
      'Оплата мобильной связи и рабочего софта',
    ],
    employment: 'Полная занятость',
    status: 'applied',
    outcome: 'won',
  ...
  },
  {
    ...
    pitch: '',
    tags: ['1C', 'ERP'],
    keySkills: ['1C:Предприятие 8.3', '1C:ERP', 'Конвертация данных', 'MS SQL Server'],
    responsibilities: ['Доработка документов и отчетов в 1C:ERP 2.5', 'Консультация пользователей склада и бухгалтерии'],
    requirements: ['Опыт программирования в среде 1С от 3 лет', 'Знание типовых конфигураций ERP'],
    conditions: ['Работа в офисе в Екатеринбурге', 'Пятидневка 9-18'],
    employment: 'Полная занятость',
    status: 'skipped',
    outcome: 'lost',
  ...
  },
  {
    ...
    pitch: '',
    tags: ['QA'],
    keySkills: ['Функциональное тестирование', 'Jira', 'TestRail', 'SQL', 'Postman'],
    responsibilities: ['Проведение ручного регрессионного тестирования', 'Составление чек-листов и тест-кейсов'],
    requirements: ['Опыт или курсы тестирования ПО', 'Внимательность к деталям'],
    conditions: ['Удаленный формат, гибкий график'],
    employment: 'Полная занятость',
    status: 'skipped',
    outcome: 'lost',
